...
class WPC_Comment_Template_Builder {
    ...
    public function get_comment_template($comment, $args, $depth) {
        ...
        if ($this->is_form_visible()) {
            ...
            $output .= '<div class="wpc-field-submit"><input type="button" name="submit" value="' . $this->wpc_options_serialized->wpc_phrases['wpc_submit_text'] . '" id="wpc_comm-' . $unique_id . '" class="wpc_comm_submit button alt"/>
							</div>';
            $output .= '<div style="clear:both"></div>';
            $output .= '</div>';
