...
class WPC_DB_Helper {
    ...
    public function wpc_add_email_notification($id, $post_id, $email) {
        ...
        $sql = $this->db->prepare("INSERT INTO `" . $this->email_notification . "` (`email`, `subscribtion_id`, `post_id`, `subscribtion_type`, `activation_key`) VALUES(%s, %d, %d, %s, %s);", $email, $id, $post_id, 'comment', $activation_key);
        $this->db->query($sql);
