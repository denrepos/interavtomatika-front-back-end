...
class PMXI_CsvParser
{
    ...
    {
		$_SESSION['allimport_translit_fields'] = array();
    ...
    function toXML( $fixBrokenSymbols = false ){
		...
		while($row = fgetcsv($res, $l, $d, $e)){
			$all_rows[] = $row;
		}
				
        foreach ($all_rows as $i => $keys) {
            ...
            if ($c == 0) {
                ...
                foreach ($keys as $key => $value) {    
                    ...
                    $translit = trim(strtolower(preg_replace('/^[0-9]{1}/','el_', preg_replace('/[^a-z0-9_]/i', '', $this->translit($value)))));
                    
					$_SESSION['allimport_translit_fields'][$translit] = $value;
	
					$keys[$key] = (!empty($translit)) ? $translit : 'undefined' . $key;
                }            
                $this->headers = $keys;                                
            ...
            if ( $c or $create_new_headers ) {
               ...
               if (!empty($keys)){                                   
                    ...
                    foreach ($this->headers as $key => $header){
						...
						for ($coll_i = $header_i+1; substr($all_rows[0][$coll_i],0,2) == '[:'; $coll_i++){
							$keys[$key] = '[:ru]'.$keys[$key].substr($all_rows[0][$coll_i],0,5).$keys[$coll_i];
						}
                    ...
                    {                                                                                        
                        ...
                        foreach ($chunk as $header => $value) 
                        {
                            $xmlWriter->startElement($header);
								...
								$value = AllImportiUnknownFields::translate($value,$_SESSION['allimport_translit_fields'][$header]);
...
PC::debug($str.'---'.$translit,'translit');
		return $translit;
	}
